...
1. ⚠️ **Finish the #39 deploy (do this before clicking tomorrow's email links):** (a) run [`backend/sql/2026-06-09_outfit_feedback.sql`](backend/sql/2026-06-09_outfit_feedback.sql) in the Supabase SQL editor — **verified NOT run as of merge** (the `feedback` column doesn't exist yet, so a 👍 click returns the 500 page); (b) add `FEEDBACK_SECRET` to the Render env, copying the value from repo-root `.env` (mismatch ⇒ every link 400s). GitHub Actions secrets are already set. Then click a thumb in the next email end-to-end.
2. **[#40](https://github.com/JiamanBettyWu/wardrobe-ai/issues/40) warmth attribute** — the other data-collection piece of the feedback-loop plan. (Then #42/#18/#44 once data and fields exist.)
3. **Check the daily emails for the #43 fix** — Elevated should now omit shoes (with a note in reasoning) rather than show sport sandals.
...
- [#44 Category-aware recency: skip recency weighting for small categories](https://github.com/JiamanBettyWu/wardrobe-ai/issues/44) (`TYPE_CATEGORIES` groundwork landed in PR #48 — import from `services/categories.py`)
