...
- **2026-07-02** — *Multi-item upload (#24) is a page-header chip toggle ("Multi-item photo") on Add item, mounting a sibling form rather than adding a mode flag to `AddClothingForm`.* Placement follows the Travel-mode rule (page-level toggles live in `.page-header__actions`), but the control is a `.chip`, not a native checkbox — the 2026-06-05 native-checkbox→chip decision applies to page headers too; the Travel-mode checkboxes are the leftover, not the precedent (tracked in #95). On-state is a **soft** fill (`--on-mode`, the attribution/laundry weight): an input mode is a standing preference you compose under, not a commitment. The chip label is short per the chip-vs-tile rule; the explanatory copy ("best for accessories", shared-photo disclosure) moved to a muted hint under the file input. Each detected item renders as a `.multi-form__card` — the content-tile glass formula (tint 0.45 / blur 20 / radius 16, same as `.outfit__item` and `.profile__pref-tile`: a review card is a "content unit") with form-scale `14px` padding since it holds inputs, not metadata. The shared photo renders **once** above the cards with a muted "N items found — all will share this photo" line, so the cosmetic tradeoff (group thumbnail on every item) is disclosed at commit time, not discovered in the Catalog. Cards save/discard independently (card-local status + error, per the persistent-`.error` rule); saved cards collapse to a muted "✓ Saved — name" line instead of disappearing, so the running tally stays visible. "Done" is ghost while cards remain and promotes to primary when all are saved — same weight-carries-meaning logic as the chip fills.
