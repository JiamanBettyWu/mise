...
- **Toggle chips** — `.chip` in [ClothingCardGlass.css](frontend/src/components/ClothingCardGlass.css). Pill (999px) that toggles an item state on/off. Off = ghost pill (translucent + faint border + hollow dot) so it still reads as pressable, not as metadata text. On-states carry meaning in fill *weight*: `--on-laundry` soft muted fill (passive), `--on-packed` strong ink fill (committed). Rendered as `<button aria-pressed>` for free focus/keyboard support. Note: needs explicit `:hover` rules — the global `button:hover` outranks a bare `.chip`.
