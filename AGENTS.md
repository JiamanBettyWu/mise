...
**Supabase.** Single-tenant, accessed with the **service_role key** server-side only. There's no row-level user context — this is a personal app. Lazy client in [`backend/db/supabase.py`](backend/db/supabase.py). **RLS is *enabled* on every table, with zero policies** — service_role has `BYPASSRLS` so the app is unaffected, while the publicly-exposed anon key is denied everything; real auth is `APP_PASSWORD` at the FastAPI layer. **Every migration that creates a table MUST include `alter table public.<t> enable row level security;`** — five tables shipped without it and went unnoticed for three months until Supabase's Security Advisor flagged them (#161). `tests/test_sql_rls.py` enforces this in CI (#163), so the rule fails on a PR rather than in a vendor email.
...
- **Backend** → Render (auto-deploy from `main`). **The service is dashboard-managed, not Blueprint-managed** — [`render.yaml`](render.yaml) is documentation that Render does *not* read, so editing it changes nothing on the deployed service. Build Command, Start Command and env vars must be changed in the Render UI; keep `render.yaml` in step so the two don't drift. (Learned the slow way in #165, where the fix had to be applied in the UI before the PR merged.)
- **One dependency source of truth: `backend/uv.lock`.** Render, both crons, the
  ...
  **`uv run --no-sync`**, because a bare `uv run` re-syncs on boot and silently
  re-installs the dev group the build just excluded. The *interpreter* is
  pinned separately, by [`backend/.python-version`](backend/.python-version) —
  see the parity gotcha below.
- **Frontend** → Vercel (auto-deploy from `main`, root = `frontend/`). Config in [`frontend/vercel.json`](frontend/vercel.json).
- **CI** → GitHub Actions runs the offline pytest suite on every PR and push to `main` ([`.github/workflows/tests.yml`](.github/workflows/tests.yml)). Pinned to **Python 3.11 via `setup-uv`**, matching `backend/.python-version` — so CI, Render and local all run the same interpreter (see the parity gotcha below). No secrets needed; the `RUN_E2E` test self-skips.
...
- **Python version parity — now pinned, in one file.** [`backend/.python-version`](backend/.python-version) holds `3.11`, and **uv reads it everywhere**: local `uv sync`, Render's build, and the three cron workflows. That closed the long-standing 3.14-local/3.11-prod split ([#13](https://github.com/JiamanBettyWu/mise/issues/13)), where PEP 649 lazy annotations on 3.14 could silently hide `NameError`s that bit on Render at import time (the schema class-order bug in PR #12). **Deleting or loosening that file silently re-opens it in the worse direction** — Render's `PYTHON_VERSION` env var does *not* constrain uv, so uv would pick the newest interpreter matching `requires-python = ">=3.11"` and put **prod** on the newer Python while CI stayed on 3.11 (caught in review on #165). CI runs 3.11 via `setup-uv`, so import-time breakage of anything the tests touch still surfaces on the PR.
...
- **After each merged PR:** (1) **append** a new dated entry at the top of `SESSIONS.md` (narrative of what shipped + PR link) and add the issue to its closed-PR ledger; (2) **refresh** `TODO.md` — update the "Current state" pointer and the "Next time I sit down" list, and remove any line that referenced the now-shipped work as still pending. **Keep the long narrative in SESSIONS.md, not TODO.md** — TODO.md stays short. The "Current state" pointer is **bullets, not prose** — Betty reads it visually. Two labelled groups, one short bullet per item, nothing longer than ~3 lines: *Shipped &lt;date&gt;* (latest session's issue → PR, one line each) and *Open manual follow-ups* (things needing a human hand). Then link to SESSIONS.md. **Durable environment facts do NOT go in TODO.md** — a fact that stays true belongs in this file, near whatever it qualifies; TODO.md is only for things that will stop being true. Putting one in both places is how they drift apart. Narrative belongs in SESSIONS.md; **do not collapse these bullets back into a paragraph.**
