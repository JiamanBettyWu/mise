...
- **After each merged PR:** (1) **append** a new dated entry at the top of `SESSIONS.md` (narrative of what shipped + PR link) and add the issue to its closed-PR ledger; (2) **refresh** `TODO.md` — update the "Current state" pointer and the "Next time I sit down" list, and remove any line that referenced the now-shipped work as still pending. **Keep the long narrative in SESSIONS.md, not TODO.md** — TODO.md stays short. The "Current state" pointer is **bullets, not prose** — Betty reads it visually. Three labelled groups, one short bullet per item, nothing longer than ~3 lines: *Shipped &lt;date&gt;* (latest session's issue → PR, one line each), *Open manual follow-ups* (things needing a human hand), and *Standing facts* (non-obvious environment truths worth not rediscovering — e.g. the Render service being dashboard-managed). Then link to SESSIONS.md. Narrative belongs in SESSIONS.md; **do not collapse these bullets back into a paragraph.**
